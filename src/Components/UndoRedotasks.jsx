export default function UndoRedotasks(props){
    ...
    return(
        <div id={"UndoRedo_Task"} style={{display:"flex"}}>
            <button onClick={onClickUndo} id={"Undo_button"} disabled={!canUndo} className={"options_button"}>Undo</button>

            <button onClick={onClickRedo} id={"Redo_button"} disabled={!canRedo} className={"options_button"}>Redo</button>
        </div>
    )
}